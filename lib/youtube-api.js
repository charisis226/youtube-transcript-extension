 ...
 */
async function fetchSegmentsFromInnertube(videoId, languageCode) {
  ...
  if (!response.ok) {
    return { segments: [], debug: `status:${response.status} body:${rawText.slice(0, 200)}` };
  }
 ...
 */
export async function extractTranscript(captionTracks, includeTimeline = false, videoId = "", fetchXml = null) {
  ...
  let debugInfo = "";
  try {
    const { segments: segs, debug } = await fetchSegmentsFromInnertube(videoId, track.languageCode);
    segments = segs;
    debugInfo = debug;
  } catch (e) {
