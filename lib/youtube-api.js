 ...
 */
export async function extractTranscript(captionTracks, includeTimeline = false, videoId = "", innertubePost = null, transcriptParams = null) {
  ...
  let debugInfo = "";
  try {
    const { segments: segs, debug } = await fetchSegmentsFromInnertube(videoId, track.languageCode, innertubePost, transcriptParams);
    segments = segs;
    debugInfo = debug;
  } catch (e) {
  ...
  if (segments.length === 0) {
    throw new Error(`자막 실패 | params:${transcriptParams ? "있음" : "없음"} | ${debugInfo}`);
  }
