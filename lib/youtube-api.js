 ...
 */
async function fetchSegmentsFromInnertube(videoId, languageCode, innertubePost = null) {
 ...
 */
export async function extractTranscript(captionTracks, includeTimeline = false, videoId = "", innertubePost = null) {
  ...
  // Primary: Innertube API (via page context if available)
  let segments = [];
  let debugInfo = "";
  try {
    const { segments: segs, debug } = await fetchSegmentsFromInnertube(videoId, track.languageCode, innertubePost);
    segments = segs;
    debugInfo = debug;
  } catch (e) {
    debugInfo = "Innertube 예외: " + e.message;
  }

  if (segments.length === 0) {
    throw new Error(`자막 실패 | videoId:${videoId} | lang:${track.languageCode} | ${debugInfo}`);
  }
