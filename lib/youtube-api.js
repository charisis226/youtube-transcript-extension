...
async function fetchSegmentsFromInnertube(videoId, languageCode) {
  ...
  if (!response.ok) throw new Error(`Innertube API 오류: ${response.status}`);
...
export async function extractTranscript(captionTracks, includeTimeline = false, videoId = "", fetchXml = null) {
  ...
  let segments = [];
  try {
    segments = await fetchSegmentsFromInnertube(videoId, track.languageCode);
  } catch {
    // fallthrough to XML method
  }
  ...
  if (segments.length === 0 && track.baseUrl) {
    ...
    try {
      ...
      segments = parseTimedText(xml).map(({ start, text }) => ({ start, text }));
    } catch {
      // ignore
    }
  }
